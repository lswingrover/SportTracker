...
export async function diffAndPush({ eventId, teamId, teamName, payload }) {
      ...
      for (const lead of SOON_LEADS_MIN) {
        if (minsUntil <= lead && minsUntil > lead - 5) {
          ...
          if (!sent.has(evtKey)) {
            events.push({
              key: evtKey,
              kind: lead === 30 ? "game-soon-30" : "game-soon-10",
              payload: {
  ...
  // 2b. Work-duty "soon" pushes (30 min, 10 min) — separate alert types
  // so athletes can opt into duty alerts independently of game alerts.
    ...
    for (const lead of SOON_LEADS_MIN) {
      if (minsUntil <= lead && minsUntil > lead - 5) {
        ...
        if (!sent.has(evtKey)) {
          events.push({
            key: evtKey,
            kind: lead === 30 ? "work-soon-30" : "work-soon-10",
            payload: {
              title: lead === 30 ? `🟡 Duty in 30 min` : `⏰ Duty in 10 min`,
